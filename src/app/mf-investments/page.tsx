...
const lineColors = {
  selectedFund: "hsl(var(--chart-1))",
  nifty: "hsl(var(--chart-2))",
};
...
function parseGrowwXLSX(workbook: XLSX.WorkBook): ParsedTransaction[] {
  ...
  for (let i = 0; i < Math.min(rawData.length, 20); i++) {
    ...
    if (
      Array.isArray(row) &&
      row.some((cell: any) =>
        String(cell).toLowerCase().includes("scheme name")
      )
    ) {
      headerRowIdx = i;
      break;
  ...
  if (headerRowIdx === -1) {
    throw new Error(
      "Could not find header row with 'Scheme Name' in the XLSX file"
    );
  }

  const headers = (rawData[headerRowIdx] as any[]).map((h: any) =>
    String(h || "").toLowerCase().trim()
  );

  const nameIdx = headers.findIndex(
    (h) => h.includes("scheme name") || h.includes("fund name")
  );
  const typeIdx = headers.findIndex(
    (h) => h.includes("transaction type") || h.includes("type")
  );
  ...
  for (let i = headerRowIdx + 1; i < rawData.length; i++) {
    ...
    const type =
      typeIdx >= 0
        ? String(row[typeIdx] || "PURCHASE").trim().toUpperCase()
        : "PURCHASE";
    const units =
      unitsIdx >= 0
        ? parseFloat(String(row[unitsIdx] || "0").replace(/,/g, ""))
        : 0;
    const nav =
      navIdx >= 0
        ? parseFloat(String(row[navIdx] || "0").replace(/,/g, ""))
        : 0;
    const amount = parseFloat(
      String(row[amountIdx] || "0").replace(/,/g, "")
    );
...
const PortfolioTooltip = ({
  ...
  if (active && payload && payload.length) {
    const dateInvestments =
      investments?.filter((inv) => inv.date === label) || [];
          ...
          <div className="mt-2 pt-2 border-t border-border">
            ...
            {dateInvestments.map((inv, idx) => (
              <p key={idx} className="text-xs text-muted-foreground">
                ₹{inv.amount.toLocaleString("en-IN")} ({inv.units} units @ ₹
                {inv.nav})
              </p>
            ))}
          </div>
...
export default function MFInvestmentsPage() {
  ...
  const filteredPortfolioTickDates = React.useMemo(() => {
    if (filteredPortfolioChartData.length <= 15)
      return filteredPortfolioChartData.map((d: any) => d.date);
    const step = Math.ceil(filteredPortfolioChartData.length / 15);
    return filteredPortfolioChartData
      .filter(
        (_: any, i: number) =>
          i % step === 0 || i === filteredPortfolioChartData.length - 1
      )
      .map((d: any) => d.date);
  ...
  const selectedFundInvestments = React.useMemo(() => {
    ...
    return portfolioInvestments.filter(
      (inv) => inv.fundName === selectedPortfolioFund
    );
  ...
  const selectedFundColor = React.useMemo(() => {
    ...
    const idx = portfolioFunds.findIndex(
      (f) => f.fundName === selectedPortfolioFund
    );
  ...
  const selectedFundDisplayName = React.useMemo(() => {
    ...
    const fund = portfolioFunds.find(
      (f) => f.fundName === selectedPortfolioFund
    );
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-xl font-semibold">Check MF Investment</h1>
            </div>
            <p className="text-sm text-muted-foreground hidden sm:block ml-auto">
              Search a fund or upload your Groww portfolio XLSX to compare
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="search" className="space-y-4 mt-6">
            {/* Search Section */}
            <div className="relative max-w-2xl" ref={resultsRef}>
          ...
          <TabsContent value="upload" className="space-y-4 mt-6">
            ...
            {!uploadedFile ? (
              <div
                className="border-2 border-dashed rounded-lg p-10 text-center cursor-pointer hover:border-primary/50 transition-colors max-w-2xl"
              ...
              <div className="flex items-center justify-between bg-muted/30 rounded-lg p-3 max-w-2xl">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      ...
                      {portfolioFunds.length > 0 &&
                        ` · ${portfolioFunds.length} funds resolved`}
                    </p>
                  </div>
                </div>
            ...
            {/* Portfolio Error */}
            {portfolioError && (
              <div className="text-sm text-destructive text-center py-2 bg-destructive/10 rounded-md px-3 max-w-2xl">
                {portfolioError}
              </div>
            )}
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1">
                      <div
                        ...
                        style={{
                          backgroundColor:
                            FUND_COLORS[idx % FUND_COLORS.length],
                        }}
                      />
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-1.5 flex-wrap">
                  {portfolioFunds.map((fund, idx) => {
                    ...
                    const isSelected =
                      selectedPortfolioFund === fund.fundName;
                    return (
                      <Button
                        key={fund.fundName}
                        ...
                        onClick={() =>
                          setSelectedPortfolioFund(fund.fundName)
                        }
                      >
                        <div
              ...
              selectedPortfolioFund && (
                <div className="flex items-center justify-between flex-wrap gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    <Button
                      variant={
                        showInvestmentMarkers ? "default" : "outline"
                      }
                      size="sm"
                      className="h-7 text-xs"
                      onClick={() =>
                        setShowInvestmentMarkers(!showInvestmentMarkers)
                      }
                    >
            ...
            {/* Portfolio Chart */}
            {!isAnalyzing &&
              filteredPortfolioChartData.length > 0 &&
              selectedPortfolioFund && (
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      ...
                      <Tooltip
                        content={
                          <PortfolioTooltip
                            investments={selectedFundInvestments}
                          />
                        }
                        cursor={{
                          stroke: "hsl(var(--muted))",
            ...
            {!isAnalyzing && !uploadedFile && !portfolioError && (
              <div className="flex flex-col items-center justify-center h-[300px] text-muted-foreground">
                ...
                <p className="text-xs mt-1">
                  Download your MF Order History from Groww and upload the
                  XLSX file
                </p>
                <p className="text-xs mt-1 opacity-60">
                  The file should have columns: Scheme Name, Transaction
                  Type, Units, NAV, Amount, Date
                </p>
              </div>
            )}
          </TabsContent>
